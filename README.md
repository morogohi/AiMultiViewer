...
| Markdown | ✅ | ✅ | 경량 렌더러 |
| PDF | ✅ | ✅ | 페이지 렌더 + 텍스트 추출 |
| DOCX | ✅(**페이지 뷰**) | ✅ | 글자 크기/굵기/기울임/색·문단 정렬·불릿·**표 그리드**·삽입 이미지를 종이 페이지 스타일로 렌더링 |
| PPTX | ✅(**슬라이드 캔버스**) | ✅ | 테마색·배경·도형 채우기(사각/원)·원본 위치·서식·이미지·표를 재현하는 시각 렌더링 + 발표자 노트 |
| XLSX | ✅(구조화) | ✅ | 시트별 **표 그리드** (열 정렬·가로 스크롤) |
| HWP | ✅(**페이지 뷰**) | ✅ | **HWP 5.0 바이너리 자체 파서** — 글자 서식·정렬·표(라벨열)·BinData 이미지(매직바이트 판별)·문단 간격을 종이 페이지 스타일로 렌더링 (암호화/DRM 제외) |
| HWPX | ✅(텍스트) | ✅ | OWPML(한글 개방형) 파싱 |
| ODT/ODS/ODP | ✅(텍스트) | ✅ | OpenDocument (Google Docs 내보내기 등) |
| Google 문서 | ✅(PDF 렌더) | ✅ | Drive에서 선택 시 PDF로 내보내 판독 |
| 이미지 (JPG/PNG 등) | ✅(줌 뷰어) | ✅ | 핀치 줌·팬·더블탭 확대 + 온디바이스 OCR(한국어+영문) 텍스트 판독 |
| DOC | ⏳ | ⏳ | 구 바이너리, 후속 지원(DOCX 권장) |

> **핀치 줌**: 모든 문서 화면에서 두 손가락 확대가 지원됩니다.
> 문서는 렌더링된 화면 그대로(이미지처럼) 확대·이동되어 확대 중에도 글자가 깨지지 않습니다.
> 두 손가락 드래그로 이동하고, 우상단 배지를 탭하면 100%로 복귀합니다.
>
> **편집**: TXT·Markdown 문서는 뷰어에서 바로 편집·저장할 수 있습니다. 그 외 문서(워드/PPT/엑셀/HWP/PDF)는 뷰어 전용입니다.
...
> 클라우드 AI는 설정에서 **ChatGPT · Claude · Gemini · Grok** 중 선택하거나 직접 입력(OpenAI 호환 서버)으로 연결합니다.
> 제공자를 선택하면 엔드포인트·기본 모델이 자동 설정되며, API Key와 모델명은 제공자별로 각각 저장됩니다.
...
## 로드맵
- ~~M2(일부): HWP 5.0 네이티브 파서~~ ✅ v0.4.0 완료 · DOC 파서는 후속
- M2(잔여): DOC(Word 97) 네이티브 파서
